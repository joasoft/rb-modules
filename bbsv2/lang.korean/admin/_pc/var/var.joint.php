...
<div id="mjointbox">
	...
	<div class="title">
		...
		<select name="where">
		<option value="name"<?php if($where == 'name'):?> selected="selected"<?php endif?>>확장게시판제목</option>
		<option value="id"<?php if($where == 'id'):?> selected="selected"<?php endif?>>확장게시판ID</option>
		</select>
		...
		&nbsp;&nbsp;
		<input type="button" value=" 새 확장게시판 " class="btn<?php echo $newboard?'gray':'blue'?>" onclick="this.form.newboard.value=1;this.form.submit();" />
		</form>
	...
	<table>
		<tr>
			...
			<td class="td2">
				...
				<span class="b">확장게시판이름</span> : 한글,영문등 자유롭게 등록할 수 있습니다.<br />
		...
		<tr>
			<td class="td1">확장게시판테마</td>
			<td class="td2">
				<select name="skin" class="select1">
				<option value="">&nbsp;+ 확장게시판 대표테마</option>
				<option value="">--------------------------------</option>
	...
	<div class="submitbox">
		<input type="submit" class="btnblue" value="새확장게시판 만들기" />
	...
	<table>
		...
		<td class="aply">
			<input type="button" value="C" title="확장게시판관리" class="btngray" onclick="window.open('<?php echo $g['s']?>/?r=<?php echo $r?>&m=admin&module=<?php echo $smodule?>&front=main');" />
		...
		<td>
			...
			<a href="<?php echo $g['s']?>/?r=<?php echo $r?>&amp;m=<?php echo $smodule?>&amp;bid=<?php echo $R['id']?>" target="_blank" title="확장게시판보기"><?php echo $R['name']?></a><span>(<?php echo $R['id']?>)</span>
			<?php endif?>
		</td>
		<td class="cat">
			<select>
			<option><?php echo $d['bbs']['skin'] ? getFolderName($tdir.$d['bbs']['skin']) : '확장게시판 대표테마'?></option>
			</select>
		</td>
		<td class="aply">
			<input type="button" value="C" title="확장게시판설정" class="btngray" onclick="window.open('<?php echo $g['s']?>/?r=<?php echo $r?>&m=admin&module=<?php echo $smodule?>&front=main&uid=<?php echo $R['uid']?>');" />
	...
	<?php else:?>
		<img src="<?php echo $g['img_core']?>/_public/ico_notice.gif" alt="" /> 아직 확장게시판이 만들어지지 않았습니다. 게시판을 만들어주세요.
	<?php endif?>
	</div>
	<?php endif?>
	<?php endif?>
...
function saveCheck(f)
{
	...
	if (f.bid.value == '')
	{
		if (f.id.value == '')
		{
			alert('확장게시판아이디를 입력해 주세요.      ');
			f.id.focus();
			return false;
		}
		if (!chkFnameValue(f.id.value))
		{
			alert('확장게시판아이디는 영문 대소문자/숫자/_ 만 사용가능합니다.      ');
			f.id.value = '';
			f.id.focus();
			return false;
		}
	}
	return confirm('정말로 새 확장게시판을 만드시겠습니까?         ');
}
//]]>
</script>